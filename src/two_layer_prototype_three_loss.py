...
class TextCNN(tf.keras.Model):
    ...
    def __init__(
        self, sequence_length, num_classes, tokenizer, bert_model, user_embeddings, topic_embeddings, embedding_size, filter_sizes, num_filters, l2_reg_lambda, dropout_keep_prob, k_protos, vect_size, l3, l4):
        ...
        self.res_distance_layer = distanceLayer()
        self.user_distance_layer = distanceLayer()
        self.conv_layers = []
        ...
        self.prototype_class_vec = np.zeros((self.k_protos, 2))
        self.num_prototypes_per_class = self.k_protos // 2
        for j in range(self.k_protos):
            self.prototype_class_vec[j, j // self.num_prototypes_per_class] = 1

        self.num_prototypes_per_class = tf.convert_to_tensor(self.num_prototypes_per_class, dtype=tf.float32)

         #hyper parameter K is the number of prototypes to push/pull during min/max loss
        if self.cohsep_ratio >= 1: #if using a flat constant for K
            self.K = int(self.cohsep_ratio)
        else: #if a ratio then calculate that number of prototypes
            self.K = int(self.num_prototypes_per_class * self.cohsep_ratio)
    ...
    def call(self, inputs):
        ...
        res_distances, res_protos = self.response_proto_layer(x)
        res_vect = self.res_distance_layer(res_distances)

        user_embeddings = self.user_embedding(input_author)

      
        user_distances, user_protos = self.user_proto_layer(tf.expand_dims(user_embeddings, axis=0))
        user_vect = self.user_distance_layer(user_distances)
        ...
        return scores, res_protos, res_distances, user_protos, user_distances
    ...
    def calculate_loss(self, distances, labels):

        distances = tf.squeeze(distances)

        # Calculate prototype labels
        proto_labels = tf.argmax(self.prototype_class_vec, axis=1)
        proto_labels = tf.cast(proto_labels, tf.int64)  # Ensure the dtype matches that of 'labels'

        labels = tf.argmax(labels, axis=1)

       
        
        # Calculate correct class similarities
        correct_class_dists = [sim[proto_labels == lab] for lab, sim in zip(labels, distances)]
        correct_class_dists = tf.stack(correct_class_dists)
        
        # Find top K least similar prototypes in its class
        least_sims_in_class = tf.math.top_k(correct_class_dists, k=self.K).values 
        cohesion_cost_example = tf.reduce_mean(least_sims_in_class, axis=1)
        cohesion_cost = tf.reduce_mean(cohesion_cost_example)
        cohesion_loss = self.l3 * cohesion_cost
        ...
        prototypes_of_correct_class = tf.gather(self.prototype_class_vec, labels, axis=1)
        prototypes_of_correct_class = tf.transpose(prototypes_of_correct_class)  
        prototypes_of_wrong_class = 1 - prototypes_of_correct_class
        
        
        prototypes_of_wrong_class = tf.cast( prototypes_of_wrong_class, tf.float32)

      

        incorrect_class_sim = distances * prototypes_of_wrong_class

        # Find top K most similar prototypes in its class
        most_sim_out_of_class = tf.math.top_k(-incorrect_class_sim, k=self.K).values *(-1)
        separation_cost_example = tf.reduce_mean(most_sim_out_of_class, axis=1)
        separation_cost = tf.reduce_mean(separation_cost_example)
        separation_loss = -1 * self.l4 * separation_cost
...
if __name__ == "__main__":
    ...
    parser.add_argument("--l3", type=float, default = 0.1)
    parser.add_argument("--l4", type=float, default = 0.1)
    ...
    out_dir = "/big/xw384/schoolwork/NLP+DEEP LEARNING/Project/CASCADE/src/runs/two_prototype_layer-15_protos-three-loss-scale-5-threshold-0.8/"
    ...
    kmedoids = KMedoids(n_clusters=args.k_protos, random_state=0).fit(sample_users_vect)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, inputs in tqdm(enumerate(train_loader)):
            ...
            with tf.GradientTape() as tape:
                predictions, res_protos, res_distances, user_protos, user_distances = ProtoCNN([x_batch, author_batch, topic_batch], training=True)
                acc_loss = criterion(y_batch, predictions)
                
                res_d = pw_distance(res_protos)
                diag_ones = tf.convert_to_tensor(np.eye(args.k_protos, dtype=float))
                diag_ones = tf.dtypes.cast(diag_ones, tf.float32)
                d1 = res_d + diag_ones * tf.reduce_max(res_d)
                d2 = tf.reduce_min(d1, axis=1)
                min_d2_dist = tf.reduce_min(d2)
                                           
                # the third loss term
                res_div_loss = tight_pos_sigmoid_offset(min_d2_dist, scale, threshold) + 1e-8

                # another loss
                res_cohesion_and_sep_loss = ProtoCNN.calculate_loss(res_distances,y_batch)

                user_d = pw_distance(user_protos)
                diag_ones = tf.convert_to_tensor(np.eye(args.k_protos, dtype=float))
                diag_ones = tf.dtypes.cast(diag_ones, tf.float32)
                d1 = user_d + diag_ones * tf.reduce_max(user_d)
                d2 = tf.reduce_min(d1, axis=1)
                min_d2_dist = tf.reduce_min(d2)
                # the third loss term
                user_div_loss = tight_pos_sigmoid_offset(min_d2_dist, scale, threshold) + 1e-8


                user_cohesion_and_sep_loss = ProtoCNN.calculate_loss(user_distances,y_batch)
    
                loss = acc_loss + args.l1*res_div_loss+args.l2*user_div_loss + user_cohesion_and_sep_loss + res_cohesion_and_sep_loss
        ...
        for x_batch, author_batch, topic_batch, y_batch in tqdm(dev_loader):
            ...
            predictions, res_protos, res_distances, user_protos, user_distances= ProtoCNN([x_batch, author_batch, topic_batch], training=False)
            acc_loss = criterion(y_batch, predictions)
            ...
            min_d2_dist = tf.reduce_min(d2)
            # the third loss term
            res_div_loss = tight_pos_sigmoid_offset(min_d2_dist, args.scale, args.threshold) + 1e-8
            ...
            min_d2_dist = tf.reduce_min(d2)
            # the third loss term
            user_div_loss = tight_pos_sigmoid_offset(min_d2_dist, args.scale, args.threshold) + 1e-8
