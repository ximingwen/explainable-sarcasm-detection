...
ProtoCNN = CNN_Bert(sequence_length=max_l,
...
out_dir = "/big/xw384/schoolwork/NLP+DEEP LEARNING/Project/CASCADE/src/runs/10_31_baseline_bert"
if not os.path.exists(out_dir):
    os.makedirs(out_dir)
...
for epoch in range(EPOCHS):
    ...
    for i, inputs, labels in train_loader:
        ...
        if (i + 1) % accumulation_steps == 0 or i == len(train_loader) - 1:
            ...
            print(f"Epoch: {epoch+1}, Loss: {accumulated_loss.numpy()/(batch_size*accumulation_steps)} {batch_size*(i+1)}/139232 train accuracy:{train_accuracy_metric.result().numpy()}")

            accumulated_loss = 0
        epoch_loss += loss

    f.write(f"Epoch: {epoch+1}, epoch Loss: {epoch_loss.numpy() }  train accuracy:{train_accuracy_metric.result().numpy()}\n")
    print(f"Epoch: {epoch+1}, epoch Loss: {epoch_loss.numpy() }  train accuracy:{train_accuracy_metric.result().numpy()}\n")
    ...
    for i, inputs, labels in valid_loader:
        ...
        if (i + 1) % accumulation_steps == 0 or i == len(valid_loader) - 1:
            ...
            print(f"Epoch: {epoch+1}, Loss: {accumulated_loss.numpy()/(batch_size*accumulation_steps)} {batch_size*(i+1)}/15488 valid accuracy:{valid_accuracy_metric.result().numpy()}")
    ...
    f.write(f"Epoch: {epoch+1}, valid Loss: {valid_loss.numpy()}  valid accuracy:{valid_accuracy_metric.result().numpy()}\n")
    print(f"Epoch: {epoch+1}, valid Loss: {valid_loss.numpy()}  valid accuracy:{valid_accuracy_metric.result().numpy()}\n")
